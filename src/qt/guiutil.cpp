...
{
    // Convert droidz:// to droidz:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("droidz://"))
    {
        uri.replace(0, 12, "droidz:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Droidz.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the IDroidzLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query IDroidzLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "droidz.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Droidz-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  droidz-qt [" + tr("command-line options") + "]                     " + "\n";
